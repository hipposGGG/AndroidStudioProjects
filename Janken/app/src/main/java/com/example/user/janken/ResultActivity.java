package com.example.user.janken;
...
public class ResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        switch (id){
            case R.id.gu:
                myHandImageView.setImageResource(R.mipmap.gu);
                myHand = JANKEN_GU;
                break;
            case R.id.choki:
                myHandImageView.setImageResource(R.mipmap.choki);
                myHand = JANKEN_CHOKI;
                break;
            case R.id.pa:
                myHandImageView.setImageResource(R.mipmap.pa);
                myHand = JANKEN_PA;
                break;
            default:
                myHand = JANKEN_GU;
        ...
        switch (comHand){
            case JANKEN_GU:
                comHandImageView.setImageResource(R.mipmap.com_gu);
                break;
            case JANKEN_CHOKI:
                comHandImageView.setImageResource(R.mipmap.com_choki);
                break;
            case JANKEN_PA:
                comHandImageView.setImageResource(R.mipmap.com_pa);
                break;
            default:
                break;
        }
    ...
    private void saveData(int myHand, int comHand, int gameResult){
        ...
        int winnigStreakCount = pref.getInt("WINNING_STREAK_COUNT",0);
        ...
        if(lastGameResult == 2 && gameResult == 2){
            //コンピュータが連勝した場合
            editor.putInt("WINNING_STREAK_COUNT", winnigStreakCount + 1);
        }else{
